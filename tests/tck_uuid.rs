...
#[then(expr = "the resulting byte sequence is {word}")]
async fn assert_protobuf(w: &mut UUIDWorld, hex_string: String) {
    let buf = hex::decode(trimhex(&hex_string)).expect("not a valid hex string");
    assert_eq!(w.protobuf, buf);
}
...
async fn main() -> Result<(), std::io::Error> {
    ...
    let out_file = opts.custom.get_junit_report_file("uuid")?;

    UUIDWorld::cucumber()
        .with_cli(opts)
        .with_writer(writer::JUnit::new(out_file, 0))
        .run("features/uuid")
        .await;
    Ok(())
}
