...
pub trait CloudEventValidator: std::fmt::Display {
    ...
    /// # Errors
    ///
    /// Returns a `CloudEventError` when one or more validations fail. The error will contain a concatenated message of all
    /// the validation errors separated by a semicolon (`;`). Each part of the message corresponds to a failure from one of
    /// the specific validation functions called within `validate`.
    /// These may include errors from:
    ///
    ...
    fn validate(&self, cloud_event: &Event) -> Result<(), CloudEventError> {
        ...
        if error_message.is_empty() {
            ...
            Err(CloudEventError::validation_error(error_message))
        }
    }
    ...
    fn validate_version(&self, cloud_event: &Event) -> Result<(), CloudEventError> {
    ...
    fn validate_id(&self, cloud_event: &Event) -> Result<(), CloudEventError> {
...
mod tests {
    ...
    fn validate_cloud_event_version_when_not_valid() {
        ...
        assert!(status
            .unwrap_err()
            .to_string()
            .contains("Invalid CloudEvent version [0.3], CloudEvent version must be 1.0",));
    }
